...
es_array_concat.$({ target: 'Array', stat: true }, {
  isArray: es_array_concat.isArray
});

var propertyIsEnumerable = es_array_concat.propertyIsEnumerableModule.f;
var createMethod = function (TO_ENTRIES) {
  return function (it) {
    var O = es_array_concat.toIndexedObject(it);
    var keys = web_domCollections_iterator.nativeKeys(O);
    ...
    while (length > i) {
      key = keys[i++];
      if (!es_array_concat.DESCRIPTORS || propertyIsEnumerable.call(O, key)) {
...
es_array_concat.$({ target: 'Object', stat: true }, {
...
var FAILS_ON_PRIMITIVES = es_array_concat.fails(function () { web_domCollections_iterator.nativeKeys(1); });
es_array_concat.$({ target: 'Object', stat: true, forced: FAILS_ON_PRIMITIVES }, {
  keys: function keys(it) {
    return web_domCollections_iterator.nativeKeys(es_array_concat.toObject(it));
  }
});

var $every = speciesConstructor.ArrayIterationModule.every;
var STRICT_METHOD = es_array_concat.arrayMethodIsStrict('every');
var USES_TO_LENGTH = es_array_concat.arrayMethodUsesToLength('every');
...
var freezing = !es_array_concat.fails(function () {
  return Object.isExtensible(Object.preventExtensions({}));
});

var internalMetadata = es_array_concat.createCommonjsModule(function (module) {
var defineProperty = es_array_concat.objectDefinePropertyModule.f;
var METADATA = es_array_concat.uid('meta');
var id = 0;
...
var fastKey = function (it, create) {
  if (!es_array_concat.isObject(it)) return typeof it == 'symbol' ? it : (typeof it == 'string' ? 'S' : 'P') + it;
  if (!es_array_concat.$has(it, METADATA)) {
...
var onFreeze = function (it) {
  if (freezing && meta.REQUIRED && isExtensible(it) && !es_array_concat.$has(it, METADATA)) setMetadata(it);
  return it;
};
...
es_array_concat.hiddenKeys[METADATA] = true;
});
...
var FAILS_ON_PRIMITIVES$1 = es_array_concat.fails(function () { nativeFreeze(1); });
es_array_concat.$({ target: 'Object', stat: true, forced: FAILS_ON_PRIMITIVES$1, sham: !freezing }, {
  freeze: function freeze(it) {
    return nativeFreeze && es_array_concat.isObject(it) ? nativeFreeze(onFreeze(it)) : it;
  }
});

var nativeIsFrozen = Object.isFrozen;
var FAILS_ON_PRIMITIVES$2 = es_array_concat.fails(function () { nativeIsFrozen(1); });
es_array_concat.$({ target: 'Object', stat: true, forced: FAILS_ON_PRIMITIVES$2 }, {
  isFrozen: function isFrozen(it) {
    return es_array_concat.isObject(it) ? nativeIsFrozen ? nativeIsFrozen(it) : false : true;
  }
});
...
var mapKeys = function mapKeys(obj, keyMapper) {
  if (!isObj(obj) || !method.isFunc(keyMapper)) return obj;
...
var deepFreeze = function deepFreeze(obj) {
  ...
  Object.getOwnPropertyNames(obj).map(function (prop) {
    obj.hasOwnProperty(prop) && obj[prop] !== null && (_typeof(obj[prop]) === 'object' || method.isFunc(obj[prop])) && !Object.isFrozen(obj[prop]) && deepFreeze(obj[prop]);
  });
  return obj;
};
var deepMerge = function deepMerge() {
  ...
  return sources.reduce(function (merged, source) {
    return array.isArr(source)
    ? [].concat(_toConsumableArray(array.isArr(merged) && merged || []), _toConsumableArray(collection.deepClone(source)))
    : isObj(source)
    ? Object.entries(source).reduce(function (joined, _ref3) {
      var _ref4 = _slicedToArray(_ref3, 2),
          key = _ref4[0],
          value = _ref4[1];
      return _objectSpread2({}, joined, _defineProperty({}, key, collection.isColl(value) && key in joined
      ? deepMerge(joined[key], collection.deepClone(value))
      : collection.deepClone(value)));
    }, merged)
    : merged;
  },
  array.isArr(sources[0]) && [] || {});
};
var applyToCloneOf = function applyToCloneOf(obj, mutatorCb) {
  ...
  if (!method.isFunc(mutatorCb)) error = 'mutator (Argument 2) arg in applyToCloneOf, must be a function!';
  if (error) return console.warn(error) || obj;
  var clone = collection.deepClone(obj);
  mutatorCb(clone);
  return clone;
};
...
var mapObj = function mapObj(obj, cb) {
  return isObj(obj) && method.isFunc(cb) && Object.entries(obj).map(function (_ref5) {
...
var mapEntries = function mapEntries(obj, cb) {
  ...
  var initialValue = array.isArr(obj) ? [] : {};
  return entries.reduce(function (obj, _ref7) {
    ...
    if (!isEntry(result)) {
      ...
      return collection.set(obj, key, value);
    }
    return collection.set(obj, result[0], result[1]);
  }, initialValue);
};
var isEntry = function isEntry(maybeEntry) {
  return array.isArr(maybeEntry) && maybeEntry.length === 2 && (number.isNum(maybeEntry[0]) || string.isStr(maybeEntry[0]));
};
...
var reduceObj = function reduceObj(obj, cb) {
  ...
  return isObj(obj) && method.isFunc(cb) && Object.entries(obj).reduce(function (data, _ref9) {
...
var sanitizeCopy = function sanitizeCopy(obj) {
  return JSON.parse(string.sanitize(JSON.stringify(obj)));
};
...
var toObj = function toObj(val, divider, split) {
  if (array.isArr(val)) return Object.keys(val).reduce(function (obj, key) {
    obj[key] = val[key];
    return obj;
  }, {});
  if (!string.isStr(str)) return {};
  divider = divider || '=';
  split = split || '&';
  return str.split(split).reduce(function (obj, item) {
    var sep = item.split(divider);
    obj[sep[0].trim()] = ext.strToType(sep[1].trim());
    return obj;
  }, {});
};
var keyMap = function keyMap(arr, toUpperCase) {
  return array.isArr(arr) && arr.reduce(function (obj, key) {
    if (!string.isStr(key)) return obj;
...
exports.FREEZING = freezing;
exports.InternalMetadataModule = internalMetadata;
